...
class Trainer(object):
    ...
    def __init__(self, in_dims, truth, num_epochs=1000, out_dir='out/', log_dir='logs/'):
        ...
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            self.optim, factor=0.5, patience=50, verbose=True)
    ...
    def fit(self, train, valid=None):
        ...
        # for j in range(self.num_epochs):
            ...
            for X, y in train_iterator:
                ...
                loss = self.l2(y_pred, Variable(y))
            ...
            s = 'After epoch {0} Test MAE: {1:0.6g} Best MAE: {2:0.6g} Test MSE: ' \
                '{3:0.6g} Best MSE: {4:0.6g} Train Loss: {5:0.6f}'.format(
                j+1, test_mae, best_mae, test_mse, best_mse, train_loss)
            tqdm.write(s, file=f1)

            self.visualize(valid, j)
    ...
    def evaluate(self, test):
        ...
        for X, y in test_iterator:
            ...
            if self.cuda_gpu:
                ...
                y = y.cuda()
            y_pred = self.model(Variable(X))
            sum_mae += self.l1(y_pred, Variable(y)).data.cpu().numpy()
            sum_mse += self.l2(y_pred, Variable(y)).data.cpu().numpy()
    ...
    def visualize(self, iterator, epoch):
...
if __name__ == '__main__':
    ...
    # Load dataset
    train = DataIterator(ddir+'data_{0}.npz'.format(exp_id), 128, shuffle=True)
    valid = DataIterator(ddir+'val.npz', 128)
    test  = DataIterator(ddir+'test.npz', 128)
    assert train.d == valid.d and train.d == test.d, \
            'Dimensions of train, valid, and test do not match!'

    # Load truth
    with np.load(ddir + 'truth.npz') as f:
        t = f['X_params']
        y = f['Y']
